...
## 🥗 FoodLens — подсчёт калорий по фото

В директории [`apps/calorie_counter`](apps/calorie_counter) доступно отдельное приложение **FoodLens**, которое с помощью
нейросети MobileNet распознаёт блюдо на фотографии и оценивает калорийность выбранной порции. Приложение работает целиком
в браузере и не требует регистрации.

### Как запустить FoodLens локально

```bash
npm install
npx http-server apps/calorie_counter -p 4173
```

Откройте http://localhost:4173 в браузере и загрузите фотографию блюда. После распознавания можно скорректировать массу
порции ползунком и получить обновлённую оценку калорийности.
